...
func TestEncodeXMLNS(t *testing.T) {
	testCases := []struct {
		...
		{encodeXMLNS3, `<ns:Test xmlns:ns="http://example.com/ns"><Body>hello world</Body></ns:Test>`, true},
...
func TestIssue7113(t *testing.T) {
	...
	wantXML := `<` + structSpace + `:A xmlns:` + structSpace + `="` + structSpace + `"><C xmlns=""></C><d></d></` + structSpace + `:A>`
	t.Log(xmlTest)
